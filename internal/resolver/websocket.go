	...
	for {
		...
		if wsErr != nil {
			if websocket.IsUnexpectedCloseError(wsErr, websocket.CloseGoingAway, websocket.CloseAbnormalClosure) {
				c.resolver.Logger.Error("error: %v", wsErr)
			}
			break
		}
		...
		if err != nil {
			c.resolver.Logger.Error("error: %v", err)
		} else {
			message = []byte{}
		}

		if dat.Payload == nil {
			c.resolver.Logger.Error("userId=%s, localChatId=%s, Payload empty, message=%s, wsErr=%s", c.userId, c.localChatId, message, wsErr.Error())
			break
		}
